...
var formResetMixin = $.ui.formResetMixin = {
	...
	_bindFormResetHandler: function() {
		...
		if ( !instances.length ) {
			...
			// We don't use _on() here because we use a single event handler per
			// form
...
var labels = $.fn.labels = function() {
	...
	// support them
	// as well as document fragments. control.labels does not work on document
	// fragments
...
if ( $.uiBackCompat !== false ) {
	$.extend( $.effects, {
		...
		createWrapper: function( element ) {
			...
			// Hotfix for jQuery 1.4 since some change in wrap() seems to
			// actually
...
$.extend( $.effects, {
	...
	// Creates a placeholder element so that the original element can be made
	// absolute
	...
	effect: function( /* effect, options, speed, callback */ ) {
		var args = _normalizeArguments.apply( this, arguments ),
			...
			prefilter = function( next ) {
				...
				// See $.uiBackCompat inside of run() for removal of defaultMode
				// in 1.13
	...
	element.animate( to, {
		...
		complete: function() {
			...
			if ( !restore ) {
				...
				// doesn't restore to the original styles from before the
				// animation.
				$.effects.saveStyle( element );
			}
 ...
 * http://trentrichardson.com/examples/timepicker Copyright (c) 2016 Trent
...
(function (factory) {
	...
	$.extend(Timepicker.prototype, {
		...
		_newInst: function ($input, opts) {
			...
			tp_inst._defaults = $.extend({}, this._defaults, inlineSettings, opts, overrides, {
				evnts: fns,
				timepicker: tp_inst // add timepicker as a property of
						...
						$tp.find('.ui_tpicker_' + litem + ' table').css({
							...
							borderCollapse: 'collapse'
								...
								var $t = $(this),
									...
									ap = h.replace(/[^apm]/ig),
									f = $t.data('for'); // loses scope, so we
														// use data-for
		...
		_onTimeChange: function () {
			...
			var hasChanged = (
						hour !== parseInt(this.hour,10) || // sliders should
															// all be numeric
						...
						(this.timezone !== null && timezone !== this.timezone.toString()) // could
																							// be
																							// numeric
																							// or
																							// "EST"
	...
	$.datepicker._getDateDatepicker = function (target, noDefault) {
		...
		if (tp_inst) {
			...
			if (tp_inst.$altInput && tp_inst._defaults.altFieldTimeOnly) {
				...
				/**
				 * in case the datetimepicker has been applied to a non-input
				 * tag for inline UI, and the user has not configured the plugin
				 * to display only time in altInput, pick current date time from
				 * the altInput (and hope for the best, for now, until "ER1" is
				 * applied)
				 * 
				 * @todo ER1. Since altInput can have a totally difference
				 *       format, convert it to standard format by reading input
				 *       format from "altFormat" and "altTimeFormat" option
				 *       values
				 */
	...
	$.timepicker.timezoneOffsetNumber = function (tzString) {
		...
		return ((normalized.substr(0, 1) === '-' ? -1 : 1) * // plus or minus
					((parseInt(normalized.substr(1, 2), 10) * 60) + // hours
																	// (converted
																	// to
																	// minutes)
	 ...
	 *            date
	 * @param {string}
	 *            fromTimezone formatted like "+0500", "-1245"
	 * @param {string}
	...
	$.timepicker.log = function () {
		// Older IE (9, maybe 10) throw error on accessing
		// `window.console.log.apply`, so check first.
...
/*
 * jQuery Masked Input Plugin Copyright (c) 2007 - 2015 Josh Bush
...
$(function() {
	...
	var calculator = {
		...
		getCaretPosition: function() {
			...
			// alert(focusOffset.left + ',' + simulatorOffset.left + ',' +
			// element.scrollLeft);
			return {
...
/*
 * ! Autosize 3.0.6 license: MIT http://www.jacklmoore.com/autosize
 */
		...
		function changeOverflow(value) {
			{
				...
				// made available by removing the scrollbar. The following
				// forces the necessary text reflow.
...
/*
 * ! Copyright (c) 2011 Brandon Aaron (http://brandonaaron.net) Licensed under
 ...
 * 
 * Copyright 2010, Tim Down Licensed under the MIT license. Version: 0.1.205
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        if (isHostProperty(testTextArea, "selectionStart") && isHostProperty(testTextArea, "selectionEnd")) {
            ...
            // Moving across a line break only counts as moving one character in
			// a TextRange, whereas a line break in
            // the textarea value is two characters. This function corrects for
			// that by converting a text offset into a
            // range character offset by subtracting one character for every
			// line break in the textarea prior to the
            // offset
 ...
 * 
 * Original jquery-browser code Copyright 2005, 2013 jQuery Foundation, Inc. and
 * other contributors http://jquery.org/license
 * 
...
(function ($) {
    ...
    mouseProto._touchEnd = function (event) {
        ...
        // If the touch interaction did not move (much), it should trigger a
		// click
        ...
        inst.dpDiv.find("[data-handler]").map(function () {
            ...
            this.updateDatePickerPosition = function (inst) {
                if (!$.datepicker._pos) { // position below input
                    ...
                    $.datepicker._pos[1] += inst.input[0].offsetHeight; // add
																		// the
																		// height
                }
